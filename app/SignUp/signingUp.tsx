...
const SigningUp = () => {
  ...
  const verifySignUp = async (formData: FormData) => {
    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      setMessage('Server not configured. Missing Supabase keys.');
      return;
    }
    ...
    try {
      ...
      setMessage(`Check email ${email} to continue sign-in process`);
    } catch (err) {
      ...
      <div className='w-full max-w-md bg-white p-8 rounded-lg shadow-md'>
        ...
        {message && (
          <p className={`mb-4 text-center ${
            message.includes('Check email') ? 'text-green-600' : 'text-red-600'
          }`}>
            {message}
          </p>
        )}
        ...
        <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full ${loading ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'} text-white py-2 px-4 rounded-md transition duration-200 mt-4`}
          >
